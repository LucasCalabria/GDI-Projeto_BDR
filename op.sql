...
SELECT CM.cpf
FROM cliente_matricula CM INNER JOIN parcelas P ON P.forma_pagamento = 'Cartão de Crédito';
...
WHERE CM.cpf IN (
    SELECT P.cpf
    FROM parcelas P
    WHERE P.forma_pagamento = 'Cartão de Crédito'
);